...
func List(client *golangsdk.ServiceClient, opts ListOptsBuilder) postpagination.Pager {
	...
	return postpagination.NewPager(client, "GET", url, nil,
		func(r postpagination.PageResult) postpagination.Page {
			p := ListPage{
				postpagination.LinkedPageBase{PageResult: r, LinkPath: []string{"security_groups_links"}},
			}
			return p
		})
}
...
func Update(client *golangsdk.ServiceClient, securityGroupId string, opts UpdateOptsBuilder) (r UpdateResult) {
	b, _ := opts.ToUpdateMap()

	_, r.Err = client.Put(UpdateURL(client, securityGroupId), b, &r.Body, &golangsdk.RequestOpts{
		OkCodes: []int{200, 201, 202, 204},
	})
	return
}
